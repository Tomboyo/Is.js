...
class Is {
  ...
  static a(v, type) {
    ...
    } else if (typeof type !== 'string' && !(type instanceof Function)) {
      throw new TypeError("Invalid type '"+type+"': must be (string, Function, Object)");
    }

    // Aliased type test
    if (type in Is.prototype.aliases && Is.prototype.aliases[type](v) === true) {
      return true;
    }
    ...
    // Instance test
    if (type instanceof Function && v instanceof Function && v instanceof type) {
      return true;
    }
